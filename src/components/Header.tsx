      ...
      <div style={{ position: 'relative' }}>
        ...
        >
          ...
          <span style={{ lineHeight: 1.15, textAlign: 'left' }}>
            ...
            <span style={{ display: 'block', fontSize: 11, color: '#94A3B8' }}>{state.currentRoleName || ROLES[state.role].label}</span>
          </span>
        </button>
        {state.userMenuOpen && (
          <div
      ...
      <Modal title="Profil Saya" onClose={() => setProfileOpen(false)} width={440} footer={<PrimaryButton onClick={() => setProfileOpen(false)}>Tutup</PrimaryButton>}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: '#64748B' }}>{state.currentRoleName || ROLES[state.role].label}</div>
          </div>
        </div>
        <ProfileRow label="Email" value={state.userEmail || 'hendra.wijaya@holdingos.co.id'} />
        <ProfileRow label="Role Aktif" value={state.currentRoleName || ROLES[state.role].label} />
